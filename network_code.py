...
st.title('团体明细网络图生成器_4月_宽口径')
group_id = int(st.text_input('请输入要查询的团体id', '2583'))
## 缓存数据
@st.cache_data
def load_data():
    try:
        author_info_df = pd.read_feather('作者信息_0420_宽口径.fth')
        data_use_df = pd.read_feather('互动明细_0420_宽口径.fth')
...
def plot_local_group_graph(G, node_df, title, edge_width_scale=1.0, figsize=(15, 10)):
    ...
    session_data = author_info[author_info['作者id'].isin(node_df['作者id'].tolist())][['作者id','30d日均总打开理由']]
    session_dict = dict(zip(session_data['作者id'],session_data['30d日均总打开理由']))
